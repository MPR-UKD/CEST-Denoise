...
def test_OF_0():
    ...
    cest_img = generate_Z_3D(img_size=(2, 2), dyn=41, ppm=4, a=1, b=0, c=1) + generate_Z_3D(img_size=(2, 2), dyn=41, ppm=4, a=0.2, b=2, c=0.5)
    cest_img -= cest_img.min()
    cest = CEST(cest=cest_img,
                wassr=wassr_img,
                mask=np.ones((2, 2)),
                cest_range=4,
                wassr_range=1,
    ...
    _, mtr_asym_img = mtr_asym(CestCurveS, np.ones((2, 2)), (1.5, 2.5), x_calcentires.max())
...
def test_OF_0_5():
    ...
    cest_img = generate_Z_3D(img_size=(2, 2), dyn=41, ppm=4, a=1, b=0.5, c=1) + generate_Z_3D(img_size=(2, 2), dyn=41, ppm=4, a=0.2, b=2, c=0.5)
    cest_img -= cest_img.min()
    cest = CEST(cest=cest_img,
                wassr=wassr_img,
                mask=np.ones((2, 2)),
