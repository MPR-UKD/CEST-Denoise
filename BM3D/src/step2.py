...
def step2_final_estimation(basic_estimate_img: np.ndarray,
    ...
    BlockSize, ThresholdDistance, MaxMatch, WindowSize, sigma, KaiserWindowBeta, mode = param

    # initialization of basic_estimate_img, weights (both np.zeros with same shape as noisy_image)
    # and the kaiser_window
    final_img, weights, kaiser_window = initialization(basic_estimate_img, BlockSize, KaiserWindowBeta)
    ...
    discrete_blocks_basic_estimate = discrete_2D_transformation(basic_estimate_img, BlockSize, mode)
    ...
    for x, y in itertools.product(
        ...
        block_positions, block_groups_basic_estimate, block_groups_noisy = \
            grouping(basic_estimate_img, ref_block_position, BlockSize, ThresholdDistance, MaxMatch, WindowSize,
                     discrete_blocks_basic_estimate, discrete_blocks_noisy)

        block_groups_noisy, WienerWeight = filtering_3D(block_groups_basic_estimate, block_groups_noisy, sigma, mode)

        aggregation(block_groups_noisy, WienerWeight, block_positions, final_img, weights,
                    kaiser_window)
...
def grouping(basic_estimate_image: np.ndarray,
    ...
    search_window = find_search_window(basic_estimate_image, ref_block_position, block_size, window_size)
    ...
    block_groups_basic = np.zeros((num_blocks_in_window, block_size, block_size), dtype=float)
    block_groups_noisy = np.zeros((num_blocks_in_window, block_size, block_size), dtype=float)
    distances = np.zeros(num_blocks_in_window, dtype=float)

    num_similar_blocks = 0

    # get current reference block from group of blocks
    reference_block = basic_estimate_image[ref_block_position[0]: ref_block_position[0] + block_size,
    ...
    for i in range(window_size - block_size + 1):
        for j in range(window_size - block_size + 1):
            ...
            block = basic_estimate_image[current_block_position[0]: current_block_position[0] + block_size,
    ...
    if num_similar_blocks <= max_number_of_similar_blocks:
        ...
        # most similar blocks
        idx = np.argpartition(distances[:num_similar_blocks], max_number_of_similar_blocks)
    ...
    for i in range(block_positions.shape[0]):
        ...
        block_groups_basic[i, :, :] = discrete_transform_blocks_basic[similar_point[0], similar_point[1], :, :]
        block_groups_noisy[i, :, :] = discrete_transform_blocks_noisy[similar_point[0], similar_point[1], :, :]
...
def filtering_3D(BlockGroup_basic: np.ndarray,
    ...
    for i in range(block_groups_noisy.shape[1]):
        for j in range(block_groups_noisy.shape[2]):
            ...
            wiener_shrinkage_coef /= (wiener_shrinkage_coef + sigma ** 2)  # pixel weight -  wiener shrinkage coefficents
...
def aggregation(block_groups_noisy,
                wiener_weight,
                block_positions,
                final_image,
                final_weight,
    ...
    for i in range(block_positions.shape[0]):
        final_image[block_positions[i, 0]:block_positions[i, 0] + block_groups_noisy.shape[1], \
        block_positions[i, 1]:block_positions[i, 1] + block_groups_noisy.shape[2]] \
            += BlockWeight * idct2D(block_groups_noisy[i, :, :])

        final_weight[block_positions[i, 0]:block_positions[i, 0] + block_groups_noisy.shape[1], \
        block_positions[i, 1]:block_positions[i, 1] + block_groups_noisy.shape[2]] += BlockWeight
