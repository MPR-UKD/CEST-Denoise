...
def initialization(Img: np.ndarray,
                   BlockSize: int,
                   Kaiser_Window_beta: float):
    """
...
def compute_distance_of_two_blocks(block_1: np.ndarray,
                                   block_2: np.ndarray) -> float:
    """
...
@jit(nopython=True)
def find_search_window(Img: np.ndarray,
                       RefPoint: tuple,
                       BlockSize: int,
                       WindowSize: int):
    window_location = np.zeros((2, 2), dtype='int16')

    window_location[0, 0] = max(0, RefPoint[0] + int((BlockSize - WindowSize) / 2))  # left-top x
    window_location[0, 1] = max(0, RefPoint[1] + int((BlockSize - WindowSize) / 2))  # left-top y
...
def discrete_2D_transformation(image: np.ndarray,
                               BlockSize: int,
                               mode: str) -> np.ndarray:
    """
    ...
    discrete_blocks = np.zeros((image.shape[0] - BlockSize,
