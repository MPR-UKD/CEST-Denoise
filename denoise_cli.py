...
def denoise_nifti(input_path, output_path, algorithm, mask_path=None, config=None):
    ...
    if config:
        ...
        config = {'BM3D': 'default_yamls/bm3d.yaml',
...
parser = argparse.ArgumentParser(description='Denoise a NIFTI image using BM3D, NLM, or PCA')
parser.add_argument('input_path',
                    type=str,
                    help='Path to the input NIFTI image')
parser.add_argument('output_path',
                    type=str,
                    help='Path to save the denoised NIFTI image')
...
parser.add_argument('--mask_path',
                    type=str,
                    help='Path to a mask NIFTI image')
...
denoise_nifti(args.input_path, args.output_path, args.algorithm, mask_path=args.mask_path, config=args.config)
