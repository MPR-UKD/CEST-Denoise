...
def calc_mtr_asym(CestCurveS, x_calcentires, mask, d_shift, f_shift, hStep):
    ...
    for i in range(rows):
        for j in range(colums):
            if mask[i, j] != 0:
                mtr_asym_curves[i, j, :], mtr_asym_img[i, j] = calc_mtr_asym_pixel(
                    entry_mtr,
                    CestCurveS[i, j, :],
                    d_shift,
                    f_shift,
...
if __name__ == "__main__":
    ...
    FF_WASSR = Path(
        r"D:\Scibo\laufende_Kooperationen\DFG_Antrag_Anja_qMRI\in_vivo5\20_WASSR_96216"
    )
    shutil.copy(FF_WASSR / "mask_D.nii.gz", Path(temp_dir.name) / "mask.nii.gz")
    FF_WASSR = Path(
        r"D:\Scibo\laufende_Kooperationen\DFG_Antrag_Anja_qMRI\in_vivo5\18_N40PD100B1_09_95700"
    )
    ...
    cest, ppm = cest.run()
    mtr_asym_curves, mtr_asym_img = calc_mtr_asym(
        cest, ppm, mask, d_shift=1, f_shift=1, hStep=hstep
    )
    fig = overlay_img(S0, mtr_asym_img)
    fig.savefig(rf"raw.png")

    # cest = nlm_CEST(cest, 12, 8, True)
    for crit in ["malinowski", "nelson", "median"]:
        cest_pca = pca(cest.copy(), crit, mask[:, :, 0])
        mtr_asym_curves, mtr_asym_img = calc_mtr_asym(
            cest_pca, ppm, mask, d_shift=1, f_shift=1, hStep=hstep
        )
        fig = overlay_img(S0, mtr_asym_img)
        fig.savefig(rf"{crit}.png")
